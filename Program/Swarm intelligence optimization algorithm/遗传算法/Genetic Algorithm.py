...
class Geaty:
    def __init__(self, target_func=lambda x: -x + 10*math.sin(2*x) + 7*math.cos(4*x), 
                 interval=[-5, 5], deciml_digits=6, chrmsome_size=20,cross_prob=1,mutate_prob=1,iter_round=100,
                 stop_threshold=1e-4,mutate_percentage=0.2,statistic_num=20
                 ):    
        ...
        self.stop_threshold = stop_threshold  # 当最优解适应度提升小于这个阈值时，停止计算
        ...
        self.best_fitness = self.individual_fitness_list.max()
        
        self.refer_solutn = self.plot_val()  # 参考解
          
    def plot_val(self):  # 绘制图片并且返回真实解
    ...
    def plot_iter(self):
        ...
        y_lable_list = ['mean_fitness','best_fitness','best_solution']
    ...
    def evaluate_fitness(self, chrmsome_list):  # 评估所有染色体个体的适应度,就是函数值,返回np.array
    ...
    def cross(self, chrom1, chrom2):  # 交叉过程 
        p = random()  # 随机概率
    ...
    def natural_select(self):
        chrosome_list = self.individual_list.copy()
        fitness_array = self.individual_fitness_list.copy()
    ...
    def evolve(self):  # 进行一次进化过程（假设有m个初始个体集合，每次取出两个进行杂交变异产生新种群，循环m/2次就认为完成了一次生产，然后按适应度进行优胜劣汰后认为完成了一次进化）
        pos_generate = 0
        # 初始化子代种群染色体集合
    ...
    def run(self):  # 完成算法运行的所有进化过程，遗传算法的运行引擎
        iter_count = 0
        best_solution_record = []
        while iter_count < self.iter_round:
            self.evolve()
            iter_count += 1
            if iter_count % (self.iter_round/self.statistic_num) == 0:
                print(r'Iter:  %d'%iter_count)
...
geat_algorythm = Geaty(interval=[-20,20],chrmsome_size=100,deciml_digits=5,cross_prob=0.9,mutate_prob=0.9,iter_round=10000,statistic_num=1000)
geat_algorythm.run()
